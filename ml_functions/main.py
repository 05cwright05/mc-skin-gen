...
MASK_IMAGE = "half-transparency-mask.png"
...
def process_image_background(event: firestore_fn.Event[firestore_fn.DocumentSnapshot]) -> None:
    logging.debug("RUNNING UPDATED VERSION — v3 @ 5:41 PM")
    ...
    try:
        ...
        logging.debug("starting skin extraction")
        img = Image.open(io.BytesIO(output)).convert("RGB")
        skin = extract_minecraft_skin(img)
        print("extracted")
        if user_email:
            print(f"Sending email to {user_email}...")
            # Convert the PIL image to bytes
            img_bytes = io.BytesIO()
            skin.save(img_bytes, format="PNG")
            img_bytes.seek(0)  # Rewind to the beginning

            send_email(user_email, display_name, img_bytes, google_password=google_password)
